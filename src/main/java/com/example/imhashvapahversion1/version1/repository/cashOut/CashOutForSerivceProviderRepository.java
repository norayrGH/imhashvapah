...
public interface CashOutForSerivceProviderRepository extends CrudRepository<CashOutForSerivceProvider,Long>{
    ...
    @Query("Select c from CashOutForSerivceProvider c INNER JOIN c.walletOut w INNER JOIN c.companySupplier s  WHERE w.inDate>=:startDate and w.inDate<=:endDate AND s.id=:companySupplierId")
    ...
    @Query("Select c from CashOutForSerivceProvider c INNER JOIN c.walletOut w INNER JOIN c.individualSupplier s  WHERE w.inDate>=:startDate and w.inDate<=:endDate AND s.id=:individualSupplierId")
    ...
    @Query("Select c from CashOutForSerivceProvider c INNER JOIN c.walletOut w INNER JOIN c.privateEntrepreneurSupplier s  WHERE w.inDate>=:startDate and w.inDate<=:endDate AND s.id=:privateEntrepreneurSupplierId")
