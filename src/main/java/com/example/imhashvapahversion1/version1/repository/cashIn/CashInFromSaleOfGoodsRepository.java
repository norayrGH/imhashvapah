...
public interface CashInFromSaleOfGoodsRepository extends CrudRepository<CashInFromSaleOfGoods,Long> {
    ...
    @Query("Select c from CashInFromSaleOfGoods c INNER JOIN c.walletIn w INNER JOIN c.companyCustomer s  WHERE w.inDate>=:startDate and w.inDate<=:endDate AND s.id=:companyCustomerId")
    ...
    @Query("Select c from CashInFromSaleOfGoods c INNER JOIN c.walletIn w INNER JOIN c.individualCustomer s  WHERE w.inDate>=:startDate and w.inDate<=:endDate AND s.id=:individualCustomerId")
    ...
    @Query("Select c from CashInFromSaleOfGoods c INNER JOIN c.walletIn w INNER JOIN c.privateEntrepreneurCustomer s  WHERE w.inDate>=:startDate and w.inDate<=:endDate AND s.id=:privateEntrepreneurCustomerId")
